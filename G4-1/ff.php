        ...
        if ($user) {
            ...
            if ($users) {
                ...
                foreach ($users as $user) {
                    echo '<li><a href="profile.php?id=',$user['user_id'],'"><img src="../images/guest.png" alt="プロフィール写真">', htmlspecialchars($user['user_name'] ?? ''), '</a></li>';
                    // ', htmlspecialchars($user['icon'] ?? 'default-icon.png'), '
                }
                echo '</ul>';
            } else {
                echo 'ユーザーが見つかりません。';
